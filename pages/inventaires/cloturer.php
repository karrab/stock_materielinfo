...
try {
    $sql = "UPDATE inventaires SET etat = 'cloture', date_fin = NOW() WHERE id = :id";
    $db->prepare($sql);
    $db->bind(':id', $id);
    $db->execute();

    $auth->logTrace($auth->getUserId(), 'inventaires', 'update', 'inventaires', $id, "Clôture inventaire");
    $_SESSION['success'] = 'Inventaire clôturé avec succès.';
