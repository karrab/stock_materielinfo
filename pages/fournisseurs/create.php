...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $db->prepare("SELECT COUNT(*) as count FROM fournisseurs WHERE nom = :nom");
    $db->bind(':nom', $nom);
    $result = $db->fetch();
    if ($result['count'] > 0) {
    ...
    if (empty($errors)) {
        try {
            $sql = "INSERT INTO fournisseurs (nom, notes) VALUES (:nom, :notes)";
            $db->prepare($sql);
            $db->bind(':nom', $nom);
            $db->bind(':notes', $notes);
            ...
            if ($db->execute()) {
                ...
                // Log de la trace
                $auth->logTrace($auth->getUserId(), 'fournisseurs', 'create', 'fournisseurs', $id, "Création: $nom");
                    ...
                    <form method="POST">
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="nom" name="nom" required
                                   ...
                                   placeholder="Ex: Fournisseur Informatique">
                        </div>

                        <div class="mb-3">
                            <label for="notes" class="form-label">Notes</label>
                            <textarea class="form-control" id="notes" name="notes" rows="4"
                                      placeholder="Notes ou description du fournisseur"><?php echo htmlspecialchars($notes ?? ''); ?></textarea>
                        </div>
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <p class="text-muted small">
                        Les fournisseurs permettent d'organiser les employés et les bureaux par département.
                    </p>
                    <p class="text-muted small">
                        Le nom du fournisseur doit être unique.
                    </p>
                </div>
            </div>
        </div>
