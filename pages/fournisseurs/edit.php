...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $db->prepare("SELECT COUNT(*) as count FROM fournisseurs WHERE nom = :nom AND id != :id");
    $db->bind(':nom', $nom);
    $db->bind(':id', $id);
    $result = $db->fetch();
    if ($result['count'] > 0) {
    ...
    if (empty($errors)) {
        try {
            $sql = "UPDATE fournisseurs SET nom = :nom, notes = :notes, updated_at = NOW() WHERE id = :id";
            $db->prepare($sql);
            $db->bind(':nom', $nom);
            $db->bind(':notes', $notes);
            $db->bind(':id', $id);
            ...
            if ($db->execute()) {
                $auth->logTrace($auth->getUserId(), 'fournisseurs', 'update', 'fournisseurs', $id, "Modification: $nom");
    ...
    <div class="row">
        ...
        <div class="col-md-4">
            <div class="card">
                ...
                <div class="card-body">
                    <p><strong>ID:</strong> <?php echo $fournisseur['id']; ?></p>
                    <p><strong>Créé le:</strong><br><?php echo date('d/m/Y à H:i', strtotime($fournisseur['created_at'])); ?></p>
                    <p><strong>Modifié le:</strong><br><?php echo date('d/m/Y à H:i', strtotime($fournisseur['updated_at'])); ?></p>
                </div>
            </div>
        </div>
    </div>
