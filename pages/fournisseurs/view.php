...
// Statistiques
$db->prepare("SELECT COUNT(*) as count FROM employes WHERE fournisseur_id = :id AND actif = 1");
$db->bind(':id', $id);
$nb_employes = $db->fetch()['count'];

$db->prepare("SELECT COUNT(*) as count FROM bureaux WHERE fournisseur_id = :id");
$db->bind(':id', $id);
$nb_bureaux = $db->fetch()['count'];

// Liste des employés
$db->prepare("SELECT * FROM employes WHERE fournisseur_id = :id AND actif = 1 ORDER BY nom, prenom");
$db->bind(':id', $id);
...
<div class="container-fluid main-container">
    <div class="row mb-4">
        <div class="col-12">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    <h2><i class="bi bi-truck"></i> Détails du fournisseur</h2>
                ...
                <div class="no-print">
                    <a href="<?php echo BASE_URL; ?>/pages/fournisseurs/edit.php?id=<?php echo $id; ?>" class="btn btn-warning">
    ...
    <div class="row">
        ...
        <div class="col-md-8">
            <div class="card mb-3">
                ...
                <div class="card-body">
                    <table class="table table-bordered">
                        ...
                        <tr>
                            <th>Nom</th>
                            <td><strong><?php echo htmlspecialchars($fournisseur['nom']); ?></strong></td>
                        </tr>
                        <tr>
                            <th>Notes</th>
                            <td><?php echo nl2br(htmlspecialchars($fournisseur['notes'] ?? '-')); ?></td>
                        </tr>
                        <tr>
                            <th>Date de création</th>
                            <td><?php echo date('d/m/Y à H:i', strtotime($fournisseur['created_at'])); ?></td>
                        </tr>
                        <tr>
                            <th>Dernière modification</th>
                            <td><?php echo date('d/m/Y à H:i', strtotime($fournisseur['updated_at'])); ?></td>
                        </tr>
                    </table>
                </div>
            </div>
            ...
            <div class="card">
                <div class="card-header">
                    <i class="bi bi-people"></i> Employés du fournisseur (<?php echo $nb_employes; ?>)
                </div>
                <div class="card-body">
                    ...
                    <?php else: ?>
                        <p class="text-muted text-center mb-0">Aucun employé dans ce fournisseur.</p>
                    <?php endif; ?>
                </div>
            </div>
        </div>
