...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Vérifier l'unicité du nom (sauf pour le sortie actuel)
    $db->prepare("SELECT COUNT(*) as count FROM sorties WHERE nom = :nom AND id != :id");
    $db->bind(':nom', $nom);
    $db->bind(':id', $id);
    $result = $db->fetch();
    if ($result['count'] > 0) {
        $errors[] = 'Un sortie avec ce nom existe déjà.';
    }

    if (empty($errors)) {
        try {
            $sql = "UPDATE sorties SET nom = :nom, notes = :notes, updated_at = NOW() WHERE id = :id";
            $db->prepare($sql);
            $db->bind(':nom', $nom);
            $db->bind(':notes', $notes);
            $db->bind(':id', $id);
            ...
            if ($db->execute()) {
                $auth->logTrace($auth->getUserId(), 'sorties', 'update', 'sorties', $id, "Modification: $nom");

                $_SESSION['success'] = 'Sortie modifié avec succès.';
                header('Location: ' . BASE_URL . '/pages/sorties/index.php');
                exit;
            }
        } catch (Exception $e) {
...
<div class="container-fluid main-container">
    <div class="row mb-4">
        <div class="col-12">
            <h2><i class="bi bi-box-arrow-up"></i> Modifier le sortie</h2>
        ...
        <div class="col-md-8">
            <div class="card">
                <div class="card-header">
                    <i class="bi bi-pencil"></i> Informations du sortie
                </div>
                <div class="card-body">
                    <?php if (!empty($errors)): ?>
                    ...
                    <form method="POST">
                        <div class="mb-3">
                            <label for="nom" class="form-label required">Nom du sortie</label>
